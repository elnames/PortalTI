...
export default function CrearTicket() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const isAvailable = await checkTicketsApiHealth();
  ...
  useEffect(() => {
    if (user) {
      ...
      // Cargar activos asignados al usuario
      cargarActivosAsignados(user.username);
    }
  }, [user]);
  ...
  const cargarActivosAsignados = async (email) => {
    ...
    try {
      ...
      // No mostrar error si no hay activos asignados
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-white to-blue-200 dark:from-blue-900 dark:via-gray-900 dark:to-blue-800 p-4">
      <div className="w-full max-w-md md:max-w-4xl p-6 rounded-2xl shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-md animate-fade-in">
        <div className="text-center mb-6">
          <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white">Crear Ticket de Soporte</h2>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
            Complete el formulario para generar un ticket de soporte técnico
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Se llena automáticamente con los datos de tu perfil
            </p>
          </div>
